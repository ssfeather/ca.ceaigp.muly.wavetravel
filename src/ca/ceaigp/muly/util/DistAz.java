...
public class DistAz
{
	...
	/**
	 * c lat1 => Latitude of first point (+N, -S) in degrees c lon1 => Longitude
	 * of first point (+E, -W) in degrees c lat2 => Latitude of second point c
	 * lon2 => Longitude of second point c c getDelta() => Great Circle Arc
	 * distance in degrees c getAz() => Azimuth from pt. 1 to pt. 2 in degrees c
	...
	public DistAz(double lat1, double lon1, double lat2, double lon2)
	{
		...
		rad = 2. * Math.PI / 360.0;
		/*
		 * c c scolat and ecolat are the geocentric colatitudes c as defined by
		 * Richter (pg. 318) c c Earth Flattening of 1/298.257 take from Bott
		 * (pg. 3) c
		 */
		sph = 1.0 / 298.257;
		...
		elon = lon2 * rad;
		/*
		 * c c a - e are as defined by Bullen (pg. 154, Sec 10.2) c These are
		 * defined for the pt. 1 c
		 */
		...
		delta = del / rad;
		/*
		 * c c Bullen, Sec 10.2, eqn 7 / eqn 8 c c pt. 1 is unprimed, so this is
		 * technically the baz c c Calculate baz this way to avoid quadrant
		 * problems c
		 */
		...
		baz = dbaz / rad;
		/*
		 * c c Bullen, Sec 10.2, eqn 7 / eqn 8 c c pt. 2 is unprimed, so this is
		 * technically the az c
		 */
	...
	public boolean equals(Object o)
	{
		...
		{
			DistAz oAz = (DistAz) o;
			if (oAz.stalat == stalat && oAz.stalon == stalon && oAz.evtlat == evtlat && oAz.evtlon == evtlon)
			{
				return true;
			}
		}
