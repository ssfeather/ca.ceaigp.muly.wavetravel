...
public class CircularBufferDataProvider extends AbstractDataProvider
{
	...
	private float[] currentXDataArray = new float[] {};

	private float[] currentYDataArray = new float[] {};
	...
	public synchronized void setCurrentYDataTimestamp(long timestamp)
	{
		...
		if (currentYDataChanged) tryToAddDataPoint();
	}
	...
	{
		double newXValue;
		if (!concatenate_data) traceData.clear();
		if (chronological)
		{
			if (xAxisDateEnabled)
			{
				if (updateMode != UpdateMode.TRIGGER) newXValue = currentYDataTimestamp;
				else newXValue = Calendar.getInstance().getTimeInMillis();
			}
			else
			{
				if (traceData.size() == 0) newXValue = 0;
				else newXValue = traceData.getTail().getXValue() + 1;
			}
		}
		else
		{
	 ...
	 *            the currentXData to set
	 */
	public synchronized void setCurrentXDataArray(float[] newValue)
	{
	 ...
	 *            the currentXData to set
	 */
	public synchronized void setCurrentYDataArray(float[] newValue)
	{
	...
	private void tryToAddDataArray()
	{
		...
		{
			case X_OR_Y:
				if ((chronological && currentYDataArrayChanged) || (!chronological && (currentXDataArrayChanged || currentYDataArrayChanged))) addDataArray();
				break;
			case X_AND_Y:
				if ((chronological && currentYDataArrayChanged) || (!chronological && (currentXDataArrayChanged && currentYDataArrayChanged))) addDataArray();
				break;
			case X:
				if ((chronological && currentYDataArrayChanged) || (!chronological && currentXDataArrayChanged)) addDataArray();
				break;
			case Y:
				if (currentYDataArrayChanged) addDataArray();
				break;
			case TRIGGER:
			default:
				break;
	...
	private void addDataArray()
	{
		...
		if (chronological)
		{
			...
			if (traceData.size() == 0) for (int i = 0; i < currentYDataArray.length; i++)
			{
				newXValueArray[i] = i;
			}
			else for (int i = 1; i < currentYDataArray.length + 1; i++)
			{
	...
	public void triggerUpdate()
	{
		...
		if (traceData.size() == 0 && !(currentYDataChanged || currentYDataArrayChanged)) return;
		if (currentYDataArray.length > 0) addDataArray();
		else addDataPoint();
	}
