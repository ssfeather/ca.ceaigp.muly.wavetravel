...
public class Axis extends LinearScale
{
	...
	public void setMinorTicksVisible(boolean minorTicksVisible)
	{
		...
		if (GraphicsUtil.isRAP()) super.setMinorTicksVisible(false);
		else super.setMinorTicksVisible(minorTicksVisible);
	}
	...
	public Dimension getPreferredSize(final int wHint, final int hHint)
	{
		...
		if (isVisible())
		{
			if (isHorizontal()) d.height += FigureUtilities.getTextExtents(title, titleFont).height;
			else d.width += FigureUtilities.getTextExtents(title, titleFont).height;
		}
		else
		{ // Not visible, flatten it to use zero height resp. width
			if (isHorizontal()) d.height = 0;
			else d.width = 0;
		}
		return d;
	}
	...
	public Range getTraceDataRange()
	{
		...
		{
			if (trace.getDataProvider() == null || !trace.isVisible()) continue;
			final Range range;
			if (isHorizontal()) range = trace.getDataProvider().getXDataMinMax();
			else range = trace.getDataProvider().getYDataMinMax();
			if (range == null) continue;
			if (Double.isInfinite(range.getLower()) || Double.isInfinite(range.getUpper()) || Double.isNaN(range.getLower()) || Double.isNaN(range.getUpper())) continue;
			if (low > range.getLower()) low = range.getLower();
			if (high < range.getUpper()) high = range.getUpper();
		}
		if (Double.isInfinite(low) || Double.isInfinite(high)) return null;
	...
	public boolean performAutoScale(final boolean force)
	{
		...
		{ // expand more space than needed
			if ((tempMin - min) < 0) tempMin -= thr;
			if ((tempMax - max) > 0) tempMax += thr;
		}
	...
	{
		if (majorGridColor == null) majorGridColor = XYGraphMediaFactory.getInstance().getColor(XYGraphMediaFactory.COLOR_GRAY);
	...
	{
		if (minorGridColor == null) minorGridColor = XYGraphMediaFactory.getInstance().getColor(XYGraphMediaFactory.COLOR_GRAY);
	...
	{
		if (autoScale) performAutoScale(false);
	}
	...
	{
		if (autoScaleThreshold > 1 || autoScaleThreshold < 0) throw new RuntimeException("The autoScaleThreshold must be a value in range [0,1]!"); //$NON-NLS-1$
	...
	{
		if (xyGraph != null) xyGraph.removeAxis(this);
		setOrientation(isYAxis ? Orientation.VERTICAL : Orientation.HORIZONTAL);
		if (xyGraph != null) xyGraph.addAxis(this);
	}
	...
	class AxisMouseListener extends MouseMotionListener.Stub implements MouseListener
	{
		...
		public void mousePressed(final MouseEvent me)
		{
			...
			if (me.button != 1 || !isValidZoomType(zoomType)) return;
			armed = true;
			// get start position
			switch (zoomType)
			{
				case RUBBERBAND_ZOOM:
					if (isHorizontal()) start = new Point(me.getLocation().x, bounds.y);
					else start = new Point(bounds.x, me.getLocation().y);
					end = null;
					break;
				case HORIZONTAL_ZOOM:
		...
		public void mouseDragged(final MouseEvent me)
		{
			...
			switch (zoomType)
			{
				case RUBBERBAND_ZOOM:
					...
					if (isHorizontal()) end = new Point(me.getLocation().x, bounds.y + bounds.height);
					else end = new Point(bounds.x + bounds.width, me.getLocation().y);
					break;
				case HORIZONTAL_ZOOM:
		...
		public void mouseReleased(final MouseEvent me)
		{
			...
			if (end == null || start == null || command == null) return;
