...
public class Trace extends Figure implements IDataProviderListener, IAxisListener
{
	...
	public enum PointStyle
	{
		NONE(Messages.PointNone), POINT(Messages.PointPoint), CIRCLE(Messages.PointCircle), TRIANGLE(Messages.PointTriangle), FILLED_TRIANGLE(
		        Messages.PointFilledTriangle), SQUARE(Messages.PointSquare), FILLED_SQUARE(Messages.PointFilledSquare), DIAMOND(Messages.PointDiamond), FILLED_DIAMOND(
		        Messages.PointFilledDiamond), XCROSS(Messages.PointCross), CROSS(Messages.ProintCross2), BAR(Messages.PointBar);
	...
	private void drawErrorBar(Graphics graphics, Point dpPos, ISample dp)
	{
		...
		switch (yErrorBarType)
		{
			...
			case MINUS:
				...
				if (yErrorBarType != ErrorBarType.BOTH) break;
			case PLUS:
		...
		switch (xErrorBarType)
		{
			...
			case MINUS:
				...
				if (xErrorBarType != ErrorBarType.BOTH) break;
			case PLUS:
	...
	private void drawYErrorArea(final Graphics graphics, final ISample predp, final ISample dp, final Point predpPos, final Point dpPos)
	{
		...
		if (predp.getYPlusError() == 0.0 && predp.getYMinusError() == 0.0 && dp.getYPlusError() == 0.0 && dp.getYMinusError() == 0.0) return;
		...
		graphics.popState();
		if (lighter != null) lighter.dispose();
	}
	...
	public void drawPoint(Graphics graphics, Point pos)
	{
		...
		if (pointStyle == PointStyle.NONE) return;
		graphics.pushState();
		...
		switch (pointStyle)
		{
			...
			case CIRCLE:
				...
				break;
			case TRIANGLE:
				graphics.drawPolygon(new int[] { pos.x - pointSize / 2, pos.y + pointSize / 2, pos.x, pos.y - pointSize / 2, pos.x + pointSize / 2,
				        pos.y + pointSize / 2 });
				break;
			case FILLED_TRIANGLE:
				graphics.fillPolygon(new int[] { pos.x - pointSize / 2, pos.y + pointSize / 2, pos.x, pos.y - pointSize / 2, pos.x + pointSize / 2,
				        pos.y + pointSize / 2 });
				break;
			case SQUARE:
			...
			case XCROSS:
				...
				break;
			case DIAMOND:
				graphics.drawPolyline(new int[] { pos.x, pos.y - pointSize / 2, pos.x - pointSize / 2, pos.y, pos.x, pos.y + pointSize / 2,
				        pos.x + pointSize / 2, pos.y, pos.x, pos.y - pointSize / 2 });
				break;
			case FILLED_DIAMOND:
				graphics.fillPolygon(new int[] { pos.x, pos.y - pointSize / 2, pos.x - pointSize / 2, pos.y, pos.x, pos.y + pointSize / 2,
	...
	protected void paintFigure(Graphics graphics)
	{
		...
		Point dpPos = null;
		hotSampleist.clear();
		if (traceDataProvider == null) throw new RuntimeException("No DataProvider defined for trace: " + name); //$NON-NLS-1$
				...
				for (int i = startIndex; i <= endIndex; i++)
				{
					...
					if (dpInXRange && valueIsNaN)
					{
						Point markPos = new Point(xAxis.getValuePosition(dp.getXValue(), false), yAxis.getValuePosition(
						        xAxis.getTickLablesSide() == LabelSide.Primary ? yAxis.getRange().getLower() : yAxis.getRange().getUpper(), false));
						graphics.setBackgroundColor(traceColor);
						graphics.fillRectangle(markPos.x - MARKER_SIZE / 2, markPos.y - MARKER_SIZE / 2, MARKER_SIZE, MARKER_SIZE);
						Sample nanSample = new Sample(dp.getXValue(), xAxis.getTickLablesSide() == LabelSide.Primary ? yAxis.getRange().getLower() : yAxis
						        .getRange().getUpper(), dp.getYPlusError(), dp.getYMinusError(), Double.NaN, dp.getXMinusError(), dp.getInfo());
					...
					if (dpInRange)
					{
						...
						if (errorBarEnabled && !drawYErrorInArea) drawErrorBar(graphics, dpPos, dp);
					}
					if (traceType == TraceType.POINT && !drawYErrorInArea) continue; // no
																					 // need
																					 // to
																					 // draw
																					 // line
					...
					if (predp == null)
					{ // No previous data point from which to
						// draw a line
						predp = dp;
					...
					if (traceType != TraceType.AREA)
					{
						...
						else if (!predpInRange || !dpInRange)
						{ // one in and
							// one out
					...
					{
						if (errorBarEnabled && drawYErrorInArea && traceType != TraceType.BAR) drawYErrorArea(graphics, predp, dp, predpPos, dpPos);
						...
						switch (traceType)
						{
							...
							case STEP_HORIZONTALLY:
							case STEP_VERTICALLY:
								if (plPolyline.size() == 0) plPolyline.addPoint(predpPos);
	...
	private ISample[] getIntersection(final ISample dp1, final ISample dp2)
	{
		if (traceType == TraceType.STEP_HORIZONTALLY)
		{
			...
			if (iy[1] != null) return iy;
			// Intersects one y axis?
			if (iy[0] != null) result[count++] = iy[0];
			...
			if (ix[1] != null) return ix;
			// Intersects one x axis?
			if (ix[0] != null) result[count++] = ix[0];
			return result;
		}
	...
	private ISample[] getStraightLineIntersection(final ISample dp1, final ISample dp2)
	{
		...
		{ // Intersection with lower xAxis
			...
			x = (ymin - y1) * dx / dy + x1;
			y = ymin;
			if (evalDP(x, y, dp1, dp2)) dpTuple[count++] = new Sample(x, y);
		...
		{ // Intersection with left yAxis
			...
			x = xmax;
			y = (xmax - x1) * dy / dx + y1;
			if (dx != 0 && evalDP(x, y, dp1, dp2)) dpTuple[count++] = new Sample(x, y);
		}
		return dpTuple;
	}
	...
	private boolean evalDP(final double x, final double y, final ISample dp1, final ISample dp2)
	{
		...
		if (!xAxis.getRange().inRange(x) || !yAxis.getRange().inRange(y)) return false;
		// Check if dp is between dp1 and dp2.
		// Could this be done without constructing 2 new Ranges?
		if (!new Range(dp1.getXValue(), dp2.getXValue()).inRange(x) || !new Range(dp1.getYValue(), dp2.getYValue()).inRange(y)) return false;
		// TODO why the ==dp1,2 test?
		final ISample dp = new Sample(x, y);
		if (dp.equals(dp1) || dp.equals(dp2)) return false;
		return true;
	}
	...
	public void setYAxis(Axis axis)
	{
		...
		if (xyGraph.getLegendMap().containsKey(axis)) xyGraph.getLegendMap().get(axis).addTrace(this);
		else
		{
	...
	public void setTraceColor(final Color traceColor)
	{
		...
		if (!errorBarColorSetFlag) errorBarColor = traceColor;
	...
	private void fireTraceNameChanged(String oldName, String newName)
	{
		...
		if (((oldName == null) && (newName == null)) || ((oldName != null) && oldName.equals(newName))) return;
	...
	public void dataChanged(IDataProvider dataProvider)
	{
		...
		if (!xRepainted && !yRepainted) repaint();
	}
	...
	private Range getIndexRangeOnXAxis()
	{
		...
		if (traceDataProvider.getSize() <= 0) return null;
		...
		if (min > traceDataProvider.getSample(traceDataProvider.getSize() - 1).getXValue() || max < traceDataProvider.getSample(0).getXValue()) return null;

		int lowIndex = 0;
		int highIndex = traceDataProvider.getSize() - 1;
		if (min > traceDataProvider.getSample(0).getXValue()) lowIndex = nearBinarySearchX(min, true);
		if (max < traceDataProvider.getSample(highIndex).getXValue()) highIndex = nearBinarySearchX(max, false);
	...
	private int nearBinarySearchX(double key, boolean left)
	{
		...
		while (low <= high)
		{
			...
			{
				if (mid > 0 && key > traceDataProvider.getSample(mid - 1).getXValue()) if (left) return mid - 1;
				else return mid;
				high = mid - 1;
			}
